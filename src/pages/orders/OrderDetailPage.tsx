...
const OrderDetailPage: React.FC = () => {
  ...
  const { data: products } = useSWR('catalog/products', () => catalogService.getProducts());
  ...
  const { 
    details, syncStatus, reset, addDetail, 
    removeDetail, updateDetail, updateAttribute, save,
    isConflict, setIsConflict
  } = useOrderOperations();

  const [isResolving, setIsResolving] = React.useState(false);

  // Sincronizar estado local con SWR cuando la data llega o cambia
  useEffect(() => {
  ...
  const handleOverwrite = async () => {
    ...
    try {
      // 1. Obtener la versión más reciente del servidor para el bypass
      const freshOrder = await ordersService.getById(id);
      // 2. Guardar usando esa versión como base (LWW forzado por el usuario)
      const updated = await save(id, freshOrder.version);
      if (updated) {
        mutateOrder(updated, false);
        toast.success('Servidor sobrescrito con éxito', { id: loadingToast });
      }
    } catch (err) {
      toast.error('Error al intentar sobrescribir', { id: loadingToast });
    } finally {
      setIsResolving(false);
    }
  };
  ...
  if (orderError) {
    ...
    return (
      <div className="p-20 text-center">
        ...
        <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-2">Error al cargar la orden</h2>
        <p className="text-slate-500 mb-8">No pudimos recuperar la información técnica en este momento.</p>
        <button onClick={() => navigate('/orders')} className="px-8 py-3 bg-slate-900 text-white font-bold rounded-2xl">
          Volver al Listado
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 p-6 md:p-12">
      <div className="max-w-6xl mx-auto">
        ...
        <OrderDetailsList
          ...
          onAdd={addDetail}
          onRemove={(clientId) => removeDetail(clientId)}
          onChange={(clientId, field, value) => updateDetail(clientId, field, value, products)}
          onAttributeChange={(clientId, attrIndex, value) => updateAttribute(clientId, attrIndex, value)}
        />
